...
void CFiguras::skybox(float largo, float altura, float profundidad, GLuint text1, GLuint text2, GLuint text3, GLuint text4)  //Funcion creacion cielo
{
...
void CFiguras::prismaBMO (float altura, float largo, float profundidad, GLuint t1, GLuint t2, GLuint t3, GLuint t4)  //Funcion creacion prisma
{
	...
	GLfloat vertice [8][3] = {
		...
		glBindTexture(GL_TEXTURE_2D, t1);   // choose the texture to use.
		//glBindTexture(GL_TEXTURE_2D, texture[filter]);   // choose the texture to use.
		glBegin(GL_POLYGON);	//Front
			//glColor3f(0.0,0.0,1.0);
			glNormal3f( 0.0f, 0.0f, 1.0f);
...
/*
void CFiguras:: elipsoide(float uiStacks, float uiSlices, float fA, float fB, float fC, GLuint text)
